...
/**
* Implementation of schema generator engine for MySQL.
*/
class System_Db_Mysqli_SchemaEngine implements System_Db_ISchemaEngine
{
    /**
    * Constructor.
    */
    ...
    {
        static $types = array( 'tiny' => 'tinyint', 'small' => 'smallint', 'medium' => 'mediumint', 'normal' => 'int', 'big' => 'bigint' );
        $type = $types[ $size ];
        if ( !$null )
            $type .= ' NOT NULL';
        if ( $autoIncrement )
            $type .= ' auto_increment';
        else if ( !$null || !is_null( $default ) )
    ...
    {
        static $types = array( 'tiny' => 'tinytext', 'normal' => 'text', 'medium' => 'mediumtext', 'long' => 'longtext' );
        $type = $types[ $size ];
        if ( $ascii )
            $type .= ' CHARACTER SET ascii COLLATE ascii_bin';
        if ( !$null )
    ...
    {
        static $types = array( 'tiny' => 'tinyblob', 'normal' => 'blob', 'medium' => 'mediumblob', 'long' => 'longblob' );
        $type = $types[ $size ];
        if ( !$null )
            $type .= ' NOT NULL';
        return $type;
    ...
    {
        $index = $unique ? 'UNIQUE ' : '';
        $index .= 'INDEX {' . $tableName . '}_' . $indexName . ' ( ' . join( ', ', $columns ) . ' )';
        return $index;
    }
